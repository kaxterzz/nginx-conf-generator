...
sudo cat > $NGINX_AVAILABLE_VHOSTS/$UP_SERVER_NAME.conf << EOF
...
sudo ln -s $NGINX_AVAILABLE_VHOSTS/$UP_SERVER_NAME.conf $NGINX_ENABLED_VHOSTS/$UP_SERVER_NAME.conf

# Check if nginx config file is okay
sudo nginx -t
